  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-800">Product Manager Dashboard</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
        ...
        <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl shadow-sm border border-blue-200 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-right">
              <p className="text-2xl font-bold text-blue-900">{stats.totalProjectsInvolved}</p>
              <p className="text-sm text-blue-600">Total Portfolio</p>
            </div>
          </div>
          <div className="flex items-center justify-between text-sm">
            <span className="text-blue-700">Managing: {stats.projectsManaged}</span>
            <span className="text-blue-600">Contributing: {stats.projectsAsConsultant}</span>
          </div>
        ...
        <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-xl shadow-sm border border-green-200 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-right">
              <p className="text-2xl font-bold text-green-900">{stats.overallProjectHealth}%</p>
              <p className="text-sm text-green-600">Portfolio Health</p>
            </div>
          </div>
          <div className="space-y-2">
            ...
            <p className="text-xs text-gray-700 leading-tight">{stats.portfolioHealthDescription}</p>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-orange-50 to-orange-100 rounded-xl shadow-sm border border-orange-200 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-right">
              <p className="text-2xl font-bold text-orange-900">{stats.upcomingDeadlines.length}</p>
              ...
              stats.upcomingDeadlines.slice(0, 2).map((deadline, index) => (
                <div key={index} className="text-xs">
                  <div className="flex items-center justify-between">
                    <span className={`font-medium ${deadline.daysLeft <= 7 ? 'text-red-700' : deadline.daysLeft <= 14 ? 'text-orange-700' : 'text-gray-700'}`}>
                      {deadline.type === 'project' ? '📋' : '📅'} {deadline.name}
                    </span>
                    <span className={`${deadline.daysLeft <= 7 ? 'text-red-600' : deadline.daysLeft <= 14 ? 'text-orange-600' : 'text-gray-600'}`}>
                      {deadline.daysLeft}d
                    </span>
                  </div>
                  {deadline.type === 'phase' && (
                    <div className="text-gray-500 truncate">in {deadline.projectName}</div>
                  )}
                </div>
              ))
            ) : (
              <div className="text-xs text-gray-600">No upcoming deadlines</div>
            )}
            {stats.upcomingDeadlines.length > 2 && (
              <div className="text-xs text-gray-500">+ {stats.upcomingDeadlines.length - 2} more</div>
            )}
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl shadow-sm border border-purple-200 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-right">
              <p className="text-2xl font-bold text-purple-900">{stats.pendingDecisions}</p>
              <p className="text-sm text-purple-600">Pending Decisions</p>
            </div>
          </div>
          <div className="space-y-2">
            <div className="flex items-center justify-between text-sm">
              <span className="text-purple-700">Blocked Allocations: {stats.blockedAllocations}</span>
              <span className="text-purple-600">At Risk Projects: {stats.crossProjectRisks}</span>
            </div>
            <div className="w-full bg-purple-200 rounded-full h-1.5">
              <div
                className={`h-1.5 rounded-full transition-all ${
                  stats.pendingDecisions === 0 ? 'bg-green-500' :
                  stats.pendingDecisions <= 3 ? 'bg-yellow-500' : 'bg-red-500'
                }`}
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200">
            <div className="px-6 py-4 border-b border-gray-200">
              <h2 className="text-lg font-semibold text-gray-900">PM Summary</h2>
              <p className="text-xs text-gray-500">Your role overview</p>
            </div>
            <div className="p-6 space-y-4">
              <div className="text-center">
                <div className="text-2xl font-bold text-blue-600">{stats.projectsManaged}</div>
                <div className="text-sm text-gray-600">Projects Managing</div>
              </div>

              <div className="text-center">
                <div className="text-2xl font-bold text-green-600">{stats.overallProjectHealth}%</div>
              ...
              {stats.pendingDecisions > 0 && (
                <div className="bg-orange-50 border border-orange-200 rounded-lg p-3">
                  <div className="text-center">
                    <div className="text-lg font-bold text-orange-700">{stats.pendingDecisions}</div>
                    <div className="text-xs text-orange-600">Requests Awaiting Approval</div>
                  </div>
                </div>
              )}
              ...
              {stats.upcomingDeadlines.length > 0 && (
                <div className="bg-red-50 border border-red-200 rounded-lg p-3">
                  <div className="text-center">
                    <div className="text-lg font-bold text-red-700">{stats.upcomingDeadlines.filter(d => d.daysLeft <= 7).length}</div>
                    <div className="text-xs text-red-600">Urgent Deadlines (≤7 days)</div>
                  </div>
                </div>
              )}
            </div>
