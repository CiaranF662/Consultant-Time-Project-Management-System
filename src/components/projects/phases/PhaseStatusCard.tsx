              ...
              {needsAttention && (
                <div className="flex flex-col gap-1">
                  ...
                  {hasRejectedAllocations && (
                    <div className="flex items-center gap-1 px-2 py-1 bg-red-100 border border-red-200 rounded-full">
            ...
            {individualAllocations.map((allocation) => (
              <div key={allocation.id}>
                <div className={`relative overflow-hidden flex items-center justify-between p-3 rounded-lg ${
                  allocation.approvalStatus === 'PENDING' ? 'bg-orange-50 border border-orange-200' :
                  allocation.approvalStatus === 'REJECTED' ? 'bg-red-50 border border-red-200' :
                  'bg-gray-50'
                }`}>
              ...
              {allocation.approvalStatus === 'REJECTED' && allocation.rejectionReason && (
                <div className="ml-3 mt-1 p-2 bg-red-50 border-l-2 border-red-400 rounded text-xs">
                  <span className="font-semibold text-red-800">Rejection Reason: </span>
                  <span className="text-red-700">{allocation.rejectionReason}</span>
                </div>
              )}
            </div>
            ))}
