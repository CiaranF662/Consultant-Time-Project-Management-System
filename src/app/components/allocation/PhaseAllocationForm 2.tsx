...
export default function PhaseAllocationForm({
  ...
  const initializeAllocations = () => {
    if (existingAllocations.length > 0) {
      ...
      setAllocations(
        existingAllocations.map((ea, index) => ({
          id: `existing-${ea.consultantId}-${index}`,
      ...
      setAllocations([{ id: `new-${Date.now()}`, consultantId: '', hours: 0 }]);
    }
  };

  const addAllocation = () => {
    setAllocations(prev => [...prev, { id: `new-${Date.now()}-${Math.random()}`, consultantId: '', hours: 0 }]);
  };
            ...
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-lg border border-blue-100">
              ...
              <div className="space-y-4">
                {allocations.map((allocation, index) => {
                  ...
                  return (
                    <div key={allocation.id} className="bg-white rounded-lg border-2 border-blue-200 p-4">
                      <div className="flex items-center gap-4">
                        <div className="flex-1">
                          ...
                          >
                            ...
                            {selectedConsultant && !availableConsultants.find(c => c.id === selectedConsultant.id) && (
                              <option key={`selected-${selectedConsultant.id}`} value={selectedConsultant.id}>
                                {selectedConsultant.name} (selected)
                              </option>
                            )}
                          </select>
                        </div>
