...
export default function ProjectDetailPage({ params }: { params: { projectId: string } }) {
  ...
  const [modalState, setModalState] = useState<{ isOpen: boolean; sprint: SprintWithDetails | null }>({
    isOpen: false,
    sprint: null,
  });
  ...
  const fetchProjectDetails = useCallback(async () => {
    try {
      ...
      setProject(null); // Set to null on error to stop loading
    }
  }, [params.projectId]);
  ...
  // This check is now safe because we wait for the session and project
  if (!session?.user) {
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
            ...
            {project.phases.map(phase => (
              <div key={phase.id} className="bg-white rounded-lg shadow-sm border border-gray-200">
                <div className="p-4 border-b flex justify-between items-center">
                  ...
                  {session.user.role === UserRole.GROWTH_TEAM && <AssignSprintsButton phaseId={phase.id} unassignedSprints={unassignedSprints} />}
                </div>
      ...
      {modalState.isOpen && (
        <LogHoursModal
          sprint={modalState.sprint!}
          projectId={project.id}
          onClose={() => {
              setModalState({ isOpen: false, sprint: null });
              fetchProjectDetails();
          }}
        />
      )}
